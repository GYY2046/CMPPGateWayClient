...
using System.Web;
//令牌桶算法实现限流
namespace SmsGatewayClient.RateLimiters
{
    /// <summary>
    ...
    public class RollingWindowThrottler
    {
        ...
        private int remainingTokens;    //当前持有的令牌数量
        ...
        {
            if (nextCheckTime > DateTime.Now.Ticks)
                return;

            while (expirationTimestampsQueue.Count > 0 && expirationTimestampsQueue.Peek() <= DateTime.Now.Ticks)
            {
